...
$total_sales = (float) (mysqli_fetch_assoc(mysqli_query($conn, "SELECT COALESCE(SUM(total), 0) AS total_sales FROM tbl_orders WHERE created_at BETWEEN '$date_from' AND '$date_to_inclusive'"))['total_sales'] ?? 0);
...
// Check for variance across all classification metrics (points, transactions, and recency)
// to prevent DivisionByZero errors in ml_helper.php during internal normalization steps.
$var_query = "
    SELECT 
        (MAX(loyalty_points) - MIN(loyalty_points)) as pts_diff,
        (MAX(t_count) - MIN(t_count)) as trans_diff,
        (MAX(last_act_ts) - MIN(last_act_ts)) as recency_diff
    FROM (
        SELECT c.loyalty_points, COUNT(lt.id) as t_count, UNIX_TIMESTAMP(MAX(lt.created_at)) as last_act_ts
        FROM customers c
        LEFT JOIN loyalty_transactions lt ON c.id = lt.customer_id
        GROUP BY c.id
    ) as metrics_dist";
$var_row = mysqli_fetch_assoc(mysqli_query($conn, $var_query));

// Logic: Only attempt classification if we have distinct values for all three features
$has_variance = ($var_row['pts_diff'] > 0) && ($var_row['trans_diff'] > 0) && ($var_row['recency_diff'] > 0);
...
$stats_summary = [
    ...
    'total_sales' => $total_sales // Add total sales to AI insight
];
...
while ($row = mysqli_fetch_assoc($res_top)) {
    if ($has_variance) {
        ...
        $row['ml_label'] = 'Standard';
        $row['ml_class'] = 'badge-secondary';
    }
    $top_customers[] = $row;
}
    ...
    <div class="main-content">
        ...
        <div class="stats-container" style="display: flex; gap: 20px; margin-bottom: 30px; flex-wrap: wrap;">
            <div class="card" style="background: white; padding: 20px; border-radius: 12px; flex: 1; min-width: 200px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
                <i class="fas fa-coins" style="font-size: 2rem; color: #27ae60;"></i>
                <h3>Total Points Issued</h3>
                <p style="font-size: 1.5rem; font-weight: bold;"><?php echo number_format($total_earned, 2); ?></p>
            </div>
            <div class="card" style="background: white; padding: 20px; border-radius: 12px; flex: 1; min-width: 200px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
                <i class="fas fa-hand-holding-heart" style="font-size: 2rem; color: #e67e22;"></i>
